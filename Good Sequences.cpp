...
int main()
{
    ...
    for(i=0;i<n;i++)
    {
       vector<ll> v;
		...
		if(isprime[a[i]]==0) v.push_back(a[i]);
		...
		for(j=2;j*j<=a[i];j++)
        {
            if(a[i]%j==0)
            {
				...
				if(isprime[j]==0) v.push_back(j);
				if(isprime[a[i]/j]!=j) v.push_back(a[i]/j);
        ...
        for(j=0;j<v.size();j++) dp[v[j]]=max(dp[v[j]],mx+1);

        for(j=0;j<v.size();j++) ans=max(dp[v[j]],ans);
