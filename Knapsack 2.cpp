...
///capacity is 1e9,profit is 100000

ll n,cap;
ll w[105],p[105];
ll dp[205][100005];

///returning minimum weight to gain profit

ll solve(ll idx,ll profit)
{
    //cout<<"BEfore "<<endl;
    //cout<<"IDX "<<idx<<" profit "<<profit<<endl;

    if(idx>=n)
    {
       // cout<<"IDX "<<idx<<" profit "<<profit<<endl;
      //  cout<<dp[idx][profit]<<endl;
        if(profit==0) return 0;
        else return 1e18;
    }

    if(dp[idx][profit]!=-1) return dp[idx][profit];


    ll ans=LLONG_MAX;
    if(profit-p[idx]>=0)  ans=min(ans,w[idx]+solve(idx+1,profit-p[idx]));
    ans=min(ans,solve(idx+1,profit));

    //cout<<"AFTEr "<<endl;
   // cout<<"IDX "<<idx<<" profit "<<profit<<endl;
    //cout<<"ANS "<<ans<<endl;

    return dp[idx][profit]=ans;
...
int main()
{
    ...
    //cout<<solve(0,totprofit)<<endl;


    for(i=totprofit;i>=0;i--)
    {
        if(solve(0,i)<=cap)
        {
            cout<<i<<endl;
