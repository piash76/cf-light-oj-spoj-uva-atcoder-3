...
///capacity is 100005,profit is 1e9

///dp[idx][wt] =maximum profit gained taking wt weight
ll knapsack(ll idx,ll wt)
{
    if(idx>=n) return 0;
    if(dp[idx][wt]!=-1) return dp[idx][wt];

    ll ans=-1;
    if(wt+w[idx]<=cap) ans=max(ans,p[idx]+knapsack(idx+1,wt+w[idx]));

    ans=max(ans,knapsack(idx+1,wt));
