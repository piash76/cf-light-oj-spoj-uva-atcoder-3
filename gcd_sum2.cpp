...
const int M= 1000010 ;
ll phi[M+5],sum[M+5],cum[M+5];

///gcd(i,n)= summation(d|n) {d*count(d) } = summation(d|n) {d*phi(n/d) }
void calculatePhi()
{
    for(ll i=1; i<=M; i++)
        phi[i] = i;
    ...
    for(ll i=1;i<=M;i++) ///divisor
    {
        for(ll j=i;j<=M;j+=i)  ///number
        {
            sum[j]+= (i*phi[j/i]);
        }
    }

    for(ll i=1;i<=M;i++) sum[i]-=i;
   cum[0]=0;
    for(ll i=1;i<=M;i++) cum[i]=cum[i-1]+sum[i];
