...
ll solve(ll idx,ll flag)
{
    ...
    if(flag==0)
    {
        if(s[idx]>=s[idx-1]) sum=min(sum,solve(idx-1,0));
        if(s[idx]>=r[idx-1]) sum=min(sum,solve(idx-1,1));
    }
    else
    {
        if(r[idx]>=s[idx-1]) sum=min(sum,c[idx]+solve(idx-1,0));
        if(r[idx]>=r[idx-1]) sum=min(sum,c[idx]+solve(idx-1,1));
    }
...
int main()
{
    ...
    ll ans=min(solve(n,0),solve(n,1));
