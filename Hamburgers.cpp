...
ll nb,ns,nc;
ll pb,ps,pc,tk;
ll bneed,sneed,cneed;

bool chek(ll num)
{
    ll b1=max(0ll,num*bneed-nb)*pb;
    ll s1=max(0ll,num*sneed-ns)*ps;
    ll c1=max(0ll,num*cneed-nc)*pc;
    if(b1+s1+c1<=tk) return true;
    else return false;
}
...
int main()
{
    ...
    while(l<=h)
    {
        ll mid=(l+h)/2; ///possible number of burger
        if(chek(mid))
        {
            ans=mid;
            l=mid+1;
